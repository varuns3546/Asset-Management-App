...
const VisualizationScreen = () => {
  ...
  useDebouncedAsync(
    async () => {
      ...
      try {
        const data = await visualizationService.getSurveyStats(selectedProject.id, user.token);
    ...
    return (
      <div className="visualization-screen">
        <div className="no-project-message">
          ...
          <p>Please select a project to view data visualizations</p>
        </div>
      </div>
    );
  }
      ...
      <div className="visualization-header">
        <div>
          ...
          <p className="header-subtitle">
            {selectedProject.title || selectedProject.name || 'Project'} - Analytics and Charts
          </p>
        </div>
      </div>
        ...
        <div className="visualization-container">
              ...
              <div className="stats-grid">
                ...
                {surveyStats && (
                  <StatCard
                    ...
                    value={`${surveyStats.summary.completionRate}%`}
                    subtitle={`${surveyStats.summary.assetsWithResponses} of ${surveyStats.summary.totalAssets} assets`}
                    icon="✓"
                  />
                )}
              </div>
          ...
          {surveyStats && (
            <div className="visualization-section">
              ...
              <div className="charts-grid">
                {surveyStats.byAssetType && surveyStats.byAssetType.length > 0 && (
                  <div className="chart-card">
                    <BarChart
                      data={surveyStats.byAssetType.map(type => ({
                        name: type.typeName,
                ...
                {surveyStats.byAttribute && surveyStats.byAttribute.length > 0 && (
                  <div className="chart-card">
                    <BarChart
                      data={surveyStats.byAttribute
                ...
                {surveyStats.timeline && surveyStats.timeline.length > 0 && (
                  <div className="chart-card">
                    <LineChart
                      data={surveyStats.timeline}
                      dataKey="count"
                      nameKey="date"
          ...
          {/* No Data Message */}
          {!loading && !surveyStats && !assetStats && !projectStats && (
