...
const Leaflet = ({ panelWidth, selectedBasemap = 'street', projectCoordinates, features = [], featureTypes = [], showLabels = true, labelFontSize = 12, labelColor = '#000000', labelBackgroundColor = 'rgba(255, 255, 255, 0.6)' }) => {
  ...
  // Reset loading state when key changes (new project) or basemap changes
  const prevCoordsRef = useRef(projectCoordinates);
  const prevBasemapRef = useRef(selectedBasemap);
  ...
  useEffect(() => {
    ...
    const basemapChanged = prevBasemapRef.current !== selectedBasemap;
    
    if (coordsChanged || basemapChanged) {
      mapReadyCalledRef.current = false;
      setIsMapLoading(true);
      prevCoordsRef.current = projectCoordinates;
      prevBasemapRef.current = selectedBasemap;
    }
  }, [projectCoordinates, selectedBasemap]);
